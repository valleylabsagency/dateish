...
export default function EntranceScreen() {
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(scrollX, { toValue: -textWidth, duration: 8000, easing: Easing.linear, useNativeDriver: true }),
        Animated.timing(scrollX, { toValue: width, duration: 0, useNativeDriver: true }),
      ]),
    ).start();
  }, [textWidth]);
        ...
        <View style={authStyles.modalOverlay}>
          ...
          >
            <View style={authStyles.sheet}>
              <Text style={authStyles.title}>{firstTime ? "Sign Up" : "Sign In"}</Text>
              <TextInput
              ...
              <TextInput
                ...
                editable={!loadingAuth}
              />
              {authError && <Text style={authStyles.error}>Authentication failed</Text>}
              <TouchableOpacity
  ...
  return (
    <View style={styles.container}>
      <ImageBackground source={require("../assets/images/entrance.png")} style={styles.background}>
        <Image
        ...
        <TouchableOpacity onPress={() => setShowAuth(true)} activeOpacity={0.8}>
          <Image
...
const styles = StyleSheet.create({
  ...
  doorSign: { position: "absolute", top: height * 0.49, width: width * 0.9, height: height * 0.2, alignSelf: "center" },
});
...
const authStyles = StyleSheet.create({
  ...
  input: { width: "100%", borderBottomWidth: 1, borderColor: "#000", marginVertical: 8, fontSize: 18, fontFamily: FontNames.MontserratRegular, padding: 5, color: "#000" },
